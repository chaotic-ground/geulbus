...
입력 소스: `[('ibus','hangul'), ('xkb','us+dvorak'), ('ibus','geulbus')]`.
증상: `geulbus` 활성 시 ProcessKeyEvent 가 **QWERTY(us) keysym** 을 받는다(물리 QWERTY-R → keyval `r` 0x72, dvorak `p` 아님). 즉 GNOME 이 ibus 엔진을 `us` XKB 레이아웃에 묶는다.

표기: **FACT** = 소스/문서 직접 인용 근거, **INFER** = 그 사실에서의 추론.

> ⚠️ 이 문서는 `research/05-wayland-xkb-keyval.md` 의 일부 결론을 **정정**한다. 05 는 "엔진 `<layout>` 은 GNOME Wayland 에서 무시된다, `<layout>default</layout>` 로 두면 사용자 XKB 를 상속한다"고 [STRONG] 으로 적었으나, 이번에 gnome-shell 49 소스(`status/keyboard.js`, `misc/keyboardManager.js`)를 직접 인용해 확인한 결과 **그 결론은 이 코드 경로에서 틀렸다.** 엔진 `<layout>`/`<layout_variant>` 는 **실제로 활성 XKB 레이아웃을 결정**하며(그래서 geulbus=us → us keysym 이 나온 것), `default` 는 "상속"이 아니라 사실상 us 로 귀결된다. 자세한 근거는 §1, §8.
...
따라서:
- geulbus 이 `<layout>us</layout>` → us 그룹 → 물리 QWERTY-R = `r`. (사용자 실측과 일치, §1.)
- 다른 `('xkb','us+dvorak')` 소스를 같이 두거나 순서를 바꿔도 **geulbus 활성 시엔 geulbus 자신의 layout(us)이 활성 그룹**이 된다. 상속 안 함. (§3)
...
- `('ibus','geulbus')` → `engineDesc.layout='us'`, `variant=''` → `xkbId = 'us'`.
...
- **FACT** `apply('default')` → `let info = this._layoutInfos['default']; if (!info) return;` → **아무 일도 안 함(return early).** 즉 geulbus(`layout=default`)로 전환해도 mutter 키맵은 **직전에 적용돼 있던 그룹 그대로 유지**된다.
...
- **INFER (강)** 그래서 GNOME/Wayland 에서 geulbus `<layout>default</layout>` 의 실제 결과: `xkbId='default'` 는 미등록 → geulbus 활성 시 `apply` no-op → **직전 활성 그룹이 그대로 남는다.** 실무상 그 "직전 그룹"은 거의 항상 us 계열(첫 등록 레이아웃/`DEFAULT_LAYOUT='us'`)이라 결과적으로 **us 처럼 보인다**. 어느 쪽이든 **dvorak 을 능동적으로 만들어 주지 않는다.** 즉 `default` 는 "사용자 dvorak 상속" ❌.
...
**INFER (확신 매우 높음, §1.2/1.3 의 직접 귀결)** `_getXkbId()` 가 `'us+dvorak'` 를 만들고, `setUserLayouts(['...','us+dvorak','...'])` 로 등록되며, `get_layout_info('us+dvorak')` 은 found(유효한 xkb 레이아웃) → geulbus 활성 시 활성 XKB 그룹 = `us(dvorak)`. → **물리 QWERTY-R 키가 keysym `p`(0x70)로 들어온다.** Ctrl/Alt+키 단축키도 dvorak 위치.
...
- **INFER (강)** 따라서 `sources` 에서 `us+dvorak` 을 geulbus 앞에 둬도, geulbus 로 전환하는 순간 활성 그룹은 geulbus 의 `xkbId`(=us). **앞 소스의 dvorak 을 상속하지 않는다.**
...
시사점: geulbus 도 사용자가 베이스 레이아웃을 고르는 설정을 두고, 그 값을 **엔진 desc 의 `<layout>`/`<layout_variant>` 로 반영**하면 일반성을 지키며 dvorak 을 얻는다.
...
- `sources = [('ibus','hangul'), ('xkb','us+dvorak'), ('ibus','geulbus')]`
- `xkb-options = @as []`
- `mru-sources`/`per-window` 는 런타임 상태.
- `localectl`: X11 keymap 미설정, `/etc/X11/xorg.conf.d/00-keyboard.conf` 부재(=시스템 기본).

옵션 b/c 직접 판정:
- **(b)** `sources` 에서 dvorak 을 geulbus 앞에 배치 → §3 대로 geulbus 활성 시 us 로 덮임. **동작 안 함.**
...
핵심: **엔진의 `<layout>`/`<layout_variant>` 가 변환 레이아웃을 결정한다**(§1,§2)는 *사실*을 이용하되, dvorak 을 코드에 박지 말고 **사용자 설정으로 그 두 필드를 채워 XML 을 (재)생성**한다. geulbus 은 이미 `install.sh` 가 `~/.config/ibus/component/geulbus.xml` 를 생성하므로(파일 헤더 주석 확인) 거기에 끼워넣으면 된다.
...
```bash
# 1) 사용자 layout/variant 를 ~/.config/ibus/component/geulbus.xml 의
...
### 6.4 검증
```bash
# geulbus 활성 상태에서 물리 QWERTY-R 을 눌렀을 때 ProcessKeyEvent keyval:
#   성공(dvorak): 0x70 ('p')
#   실패(us):     0x72 ('r')
# geulbus ProcessKeyEvent 진입부 keyval 로깅으로 확인 (05/06 의 GEULBUS_DEBUG_KEYS 방식).
```

### 6.5 한 단계 더: 진짜 일반화 (옵션 d 완성)
geulbus-setup(GTK 설정창)에 "베이스 키보드 레이아웃" 드롭다운(us / us+dvorak / us+colemak / ...)을 추가하고, 저장 시 컴포넌트 XML 의 layout/variant 를 다시 써서 ibus 재시작 + 재로그인. dvorak 하드코딩이 전혀 없음.
...
| (b) | 엔진 `default`, `sources` 에 dvorak 을 앞 배치 | ❌ geulbus 활성 시 us 로 덮임 | ✅ | **안 됨** |
...
- **FACT (로컬 파일)**: geulbus.xml=`<layout>us</layout>`, hangul.xml=`kr`/`kr104`, m17n/typing-booster 동적엔진+자체 스키마, gsettings 현재값, localectl 시스템 기본.
...
- 엔진 이름: 기본 `geulbus`(layout=us), 변형 `geulbus:us:<variant>`(예 `geulbus:us:dvorak`).
  Factory 는 `geulbus` 또는 `geulbus:` 접두를 모두 수락(`create_engine`). 조합 로직은 동일,
...
- `install.sh` 가 이 목록으로 `<engine>` 블록들을 생성. longname `Geulbus`/`Geulbus (Dvorak)` 등.
- 폐기: `base_layout`/`base_variant`(config.ini), `scripts/geulbus-apply-layout`(pkexec 헬퍼),
  설정창의 "단축키 키보드 배열" 드롭다운/적용 버튼. (§6 레시피는 이 방식으로 대체됨.)
- 실기 검증: 17개 엔진 등록 확인, 데몬이 각 엔진의 layout/variant 를 올바로 보고
  (`geulbus:us:colemak`→variant=colemak 등; 데몬 완전 재시작 후 정확).
...
(재로그인까지 했는데) `Geulbus (Dvorak)` 활성 시 물리 QWERTY-R 이 여전히 `r`. dvorak 안 먹음.
...
```
geulbus:us:dvorak:  .variant = undefined,  .layout_variant = "dvorak",  .layout = "us"
...
- 검증 명령: `python3 /tmp/geulbus_dump_engines.py`(데몬 desc), `gjs /tmp/probe_enginedesc.js`(_getXkbId 재현).
  수정 후: `geulbus:us:dvorak | layout='us+dvorak'`, `_getXkbId()="us+dvorak"`. ✅
