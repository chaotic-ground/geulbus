...
`nalgaeset-no-namespace.xsd` 는 geulbus 이 읽는 자판 설정(`layout.xml`, 날개셋
종합 설정 `.set` 형식)을 기계 검증하기 위한 XML Schema 다. 런타임이 아니라
개발/CI 린트에서만 쓴다(`crates/geulbus-core/tests/schema.rs`).
...
검증 린트(`crates/geulbus-core/tests/schema.rs`)는 `xmllint`(libxml2)가 있을 때만
돈다. 없으면 조용히 skip 하되 stderr 로 알리며, CI 에서 강제하려면
`GEULBUS_REQUIRE_XMLLINT=1` 로 돌려 xmllint 부재 시 실패하게 한다.
