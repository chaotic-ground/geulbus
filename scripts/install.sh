...
#   - 자판(입력 설정)을 ~/.config/geulbus/layout.xml 에 두고(사용자),
#     시스템 기본값 /usr/local/share/geulbus/layout.xml 로도 복사(sudo, fallback).
#   - ibus 컴포넌트를 /usr/share/ibus/component/geulbus.xml 로 생성(sudo).
...
# 인자가 없으면 기존 ~/.config/geulbus/layout.xml(또는 옛 이름 nalgaeset.xml)를 쓴다.
set -euo pipefail
...
geulbus_cfg_dir="$config_dir/geulbus"
cfg_dst="$geulbus_cfg_dir/layout.xml"
cfg_legacy="$geulbus_cfg_dir/nalgaeset.xml" # 옛 이름(하위 호환)
sys_cfg_dst="/usr/local/share/geulbus/layout.xml" # 시스템 기본값(fallback)
bin_dst="/usr/local/bin/geulbus"
setup_dst="/usr/local/bin/geulbus-setup"
setup_src="$repo_root/crates/geulbus/data/geulbus-setup.py"
desktop_dst="/usr/share/applications/ibus-setup-geulbus.desktop"
desktop_src="$repo_root/crates/geulbus/data/ibus-setup-geulbus.desktop"
component_dst="/usr/share/ibus/component/geulbus.xml"

# 레이아웃별 엔진 목록: "엔진이름접미|layout|variant|표시이름".
# 접미가 비면 기본 엔진 'geulbus'(QWERTY). 그 외엔 'geulbus:us:VARIANT'.
...
echo "[1/5] 릴리스 빌드"
cargo build --release -p geulbus
bin_src="$repo_root/target/release/geulbus"
...
for spec in "${engines[@]}"; do
  ...
  name="geulbus${suffix}"
  # 기본 엔진만 longname 'Geulbus', 나머지는 'Geulbus (배열)'.
  if [[ -z "$suffix" ]]; then
    longname="Geulbus"
  else
    longname="Geulbus (${disp})"
  fi
  ...
  engine_xml+="
    <engine>
      ...
      <description>Geulbus 한글 입력기 (${disp})</description>
      <rank>50</rank>
      <symbol>가</symbol>
...
<component>
  ...
  <homepage>https://github.com/chaotic-ground/geulbus</homepage>
...
sleep 2
if ibus list-engine 2>/dev/null | grep -qi geulbus; then
  echo "      OK: geulbus 엔진들이 등록됨."
  echo "      입력 소스 추가: GNOME 설정 → 키보드 → 입력 소스 → 한국어에서"
  echo "      'Geulbus' / 'Geulbus (Dvorak)' / 'Geulbus (Colemak)' 등 원하는 배열 선택."
  echo "      (자판 배열이 곧 단축키·영문 배열입니다. 한글 자판은 어느 것이든 동일.)"
else
  echo "      경고: list-engine 에 geulbus 이 안 보임. 로그아웃/로그인 후 재시도." >&2
fi
