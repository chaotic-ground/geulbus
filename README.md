...
- **한글 입력은 키보드 레이아웃과 무관합니다.** 자판을 물리 키 위치(keycode)로 잡으므로,
  ...
  방식과 동일).
- **단축키(Ctrl/Alt/Super+키)는 그대로 통과시킵니다.** 단축키의 배열은 입력기가 아니라
  사용자의 키보드 레이아웃(XKB)이 정합니다. 드보락 단축키를 원하면 시스템/입력 소스의
  키보드 배열을 드보락으로 두세요. (Wayland 에서는 IME 가 단축키 위치를 바꿀 수 없습니다 —
  자세한 근거는 `research/05-wayland-xkb-keyval.md`, `research/06-keyboard-layout-decision.md`.)
