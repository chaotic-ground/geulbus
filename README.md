...
- [x] 한/영 전환(한글 키 / CapsLock, ShortcutTable 해석) + 패널 표시기(날개셋 방식 `가`/`A`)
...
- 한/영은 **한글 키** 또는 설정 `ShortcutTable` 의 `IME_SWITCH` 글쇠로 전환합니다.
- 패널 표시기는 날개셋(Windows) 방식을 따라 **한글 모드 `가` / 영문 모드 `A`** 로 동적
  표시됩니다(ibus 속성 `RegisterProperties`/`UpdateProperty`).
