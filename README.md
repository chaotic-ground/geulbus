...
- [x] 한/영 전환(한글 키 / CapsLock, ShortcutTable 해석) + 패널 표시기(날개셋 방식 `글N`/`AN`)
...
#   - 자판     → ~/.config/presguel/layout.xml  (인자로 경로 지정 가능)
#               + 시스템 기본값 /usr/local/share/presguel/layout.xml (fallback)
...
- 패널 표시기는 날개셋(Windows) 방식을 따라 **`접두 + 항목번호`** 로 동적 표시됩니다
  (한글 항목 `글N`, 로마자/직접 항목 `AN`; 예 `글0`, `A1`). 항목을 XML 에 추가하면
...
자판 파일은 다음 순서로 찾습니다(존재하는 첫 파일): `PRESGUEL_CONFIG` 환경변수 →
`~/.config/presguel/layout.xml` → `~/.config/presguel/nalgaeset.xml`(옛 이름, 하위 호환)
→ `<XDG_DATA_DIRS>/presguel/layout.xml`(시스템 기본값).
...
- **단축글쇠 사용** 토글: 끄면 한/영 키 등 전환 글쇠를 엔진이 가로채지 않고 통과시켜,
  ...
  합니다.)
- 영문 배열·단축키 배열은 GNOME 입력 소스에서 `Presguel (Dvorak)` 처럼 배열별 항목을 골라
